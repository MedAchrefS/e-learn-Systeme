...
router.get('/manage', function(req, res, next){
	Instructor.getInstructorByUsername(req.user.username, function(err, instructor){
        ...
        if(instructor.gender=='male')
         ...
         female=true;
		res.render('instructor/ManegeAccount', {instructor: instructor,male: male, female: female});
	});
});
router.post('/manage', function(req, res, next){
    var info = [];
	info['instructor_username'] = req.params.instructor_username;
	info['instructor_email'] = req.body.instructor_email;
	info['gender'] = req.body.gender;
	

	Class.addLesson(info, function(err, lesson){
		console.log('Modification Hase been saved.');
	});

	req.flash('success_msg','Modification has been saved');
	res.redirect('/instructors/classes');
});
